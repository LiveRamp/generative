...
public class AbitraryUtil {
  @NotNull
  public static <T, C extends Collection<T>> List<C> shrinkCollection(C val, Arbitrary<T> internal, Function<List<T>, C> fn, int length) {
    List<List<T>> shrinkElements = val.stream().limit(length)
        .map(v -> {
